...
\begin{document}
	\begin{tikzpicture}[node distance=3cm, ->]
		\node[state, initial] (s0) {\textcolor{unifiRed}{$s_0$}};
		\node[state, right=of s0] (s1) {$s_1$};
		\node[state, right=of s1] (h){$s_\mathrm{h}$};
		\draw	
			(s0) edge[loop above] node[align=center] {$\textcolor{unifiRed}{\nonempty(q_1)\land\tos(q_1)}$\\$:(\pop,\pushone)$} (s0)
			(s0) edge[loop below] node[align=center] {$\textcolor{unifiRed}{\nonempty(q_1)\land\lnot\tos(q_1)}$\\$:(\pop,\noop)$} (s0)
